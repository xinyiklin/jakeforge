...
type Action =
  ...
  | { type: "addSection"; sectionType: ResumeSectionType; atStart?: boolean }
...
function reduce(data: ResumeData, action: Action): ResumeData {
  switch (action.type) {
    ...
    case "addSection": {
      ...
      return { ...data, sections: action.atStart ? [section, ...data.sections] : [...data.sections, section] };
    }
    case "removeSection":
...
export function useResumeEditor() {
  ...
  const actions = useMemo(
    () => ({
      ...
      addSection: (sectionType: ResumeSectionType, atStart = false) => dispatch({ type: "addSection", sectionType, atStart }),
