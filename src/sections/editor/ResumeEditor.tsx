...
export function ResumeEditor({ data, actions, style }: ResumeEditorProps) {
  ...
  // Second add-section entry point: a "+" pinned to the page's top-right corner,
  // so a section can be added without scrolling to the bottom button.
  ...
  useEffect(() => {
    ...
    function onPointerDown(event: PointerEvent) {
      if (cornerRef.current && !cornerRef.current.contains(event.target as Node)) setCornerOpen(false);
    }
    function onKeyDown(event: KeyboardEvent) {
      if (event.key === "Escape") setCornerOpen(false);
    }
  ...
  function addSectionOfType(type: ResumeSectionType, atStart: boolean) {
    actions.addSection(type, atStart);
    setPickingType(false);
    setCornerOpen(false);
  }

  // Shared section-type picker options. The bottom button appends; the top-right
  // corner "+" prepends (atStart) so each adds at its own end of the page.
  const renderSectionOptions = (atStart: boolean) => (
    <>
      <button type="button" className="rdx-add-section__option" onClick={() => addSectionOfType("summary", atStart)}>
      ...
      <button type="button" className="rdx-add-section__option" onClick={() => addSectionOfType("standard", atStart)}>
        <span className="rdx-add-section__option-title">Bulleted entries</span>
        <span className="rdx-add-section__option-sub">Title, dates &amp; bullet points — experience, projects, education</span>
      </button>
      <button type="button" className="rdx-add-section__option" onClick={() => addSectionOfType("skills", atStart)}>
  ...
  return (
    <article className="resume-doc resume-doc--editable resume-doc--paged" style={editorStyle} ref={docRef}>
      ...
      <div className={`rdx-add-corner${cornerOpen ? " is-open" : ""}`} ref={cornerRef}>
        <button
          type="button"
