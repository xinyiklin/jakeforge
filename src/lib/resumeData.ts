...
// A standalone line that names a common resume section — matched case-insensitively
// so Title-Case ("Education") and lowercase headings are detected, not just the
// ALL-CAPS form jakeforge's own exports use. Anchored to the whole line, so a
// skills sub-label like "Languages: Python, …" (has a colon + list) never matches.
// "languages" is intentionally excluded — it reads far more often as a skill label.
const KNOWN_SECTION_RE =
  /^(?:education|experience|work[ -]?experience|professional[ -]experience|employment(?:[ -]history)?|projects?|technical skills|skills(?: (?:&|and) (?:interests|tools))?|core competencies|competencies|summary|professional summary|career summary|objective|profile|about(?: me)?|certifications?|licenses?(?: (?:&|and) certifications?)?|awards?|honors?|awards? (?:&|and) honors?|honors? (?:&|and) awards?|publications?|research(?: experience)?|volunteer(?:ing| experience)?|leadership(?: (?:&|and) activities)?|activities|extracurriculars?|extracurricular activities|interests|references|relevant coursework|coursework|achievements|accomplishments)$/i;

// Strip a trailing colon (e.g. "EDUCATION:" / "Skills :") — common in resumes,
// and never part of the heading itself.
function stripHeadingColon(line: string): string {
  return line.trim().replace(/\s*:\s*$/, "");
}

function isSectionHeader(line: string): boolean {
  const trimmed = stripHeadingColon(line);
  if (trimmed.length < 2 || trimmed.length > 50) return false;
  // A known section name (any case) is a heading regardless of capitalization.
  if (KNOWN_SECTION_RE.test(trimmed)) return true;
