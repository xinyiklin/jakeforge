...
const DEFAULTS = {
  ...
  // Extra space on the `\\` break between skill lines (0 = upstream — plain `\\`).
  skillLineGapPt: 0,
  ...
  italicDates: true,
  // Section-heading look — Jake's upstream is small-caps (\scshape) + a titlerule.
  uppercaseHeadings: true,
  sectionRule: true,
  // LaTeX-ready contact separator (upstream is $|$).
  contactDivider: "$|$"
};

// Map a user-typed 1–2 char divider to safe LaTeX. Known glyphs that need math
// mode or commands are mapped; everything else is plain-escaped.
function latexDivider(div) {
  const d = (typeof div === "string" ? div : "|").trim();
  if (!d) return "\\;"; // blank → just a thin space between items
  const map = { "|": "$|$", "•": "$\\bullet$", "·": "$\\cdot$", "∙": "$\\cdot$", "–": "--", "—": "---" };
...
function styleToLatex(docStyle) {
  ...
  // Skill lines break with `\\` (no natural item separation), so the full
  // bulletGap becomes the extra space — mirrors the editor's per-skill-row gap.
  const skillLineGapPt = Math.max(0, Math.round(bulletGapEm * 11));
  ...
  return {
    ...
    contactDivider: latexDivider(docStyle.contactDivider)
  };
}
...
\titleformat{\section}{\vspace{${fmtPt(style.sectionPreVSpacePt)}}${style.uppercaseHeadings ? "\\scshape" : ""}\raggedright\large${style.boldHeadings ? "\\bfseries" : ""}}{}{0em}{}[\color{black}${style.sectionRule ? "\\titlerule " : ""}\vspace{${fmtPt(style.sectionPostVSpacePt)}}]
...
function renderHeader(resume, style) {
  ...
  return `\\begin{center}
    ...
    \\small ${contactPieces.join(` ${style.contactDivider} `)}
\\end{center}`;
}
...
// The `\\` line break between skill lines, carrying the bulletGap-derived extra
// vertical space so the PDF's skill spacing tracks the editor's. 0pt → plain `\\`
// (upstream-identical).
function skillBreak(style) {
  return style.skillLineGapPt > 0 ? `\\\\[${style.skillLineGapPt}pt]` : `\\\\`;
}
...
function renderItem(item, style) {
  ...
  if (isSkillsLike) {
    ...
    return `    \\small{\\item{
${item.bullets.map((bullet) => `        ${renderSkillBullet(bullet, style.boldSkillLabels)} ${skillBreak(style)}`).join("\n")}
    }}`;
  }
...
${paragraphs.map((text) => `  {\\small ${escapeTex(text)}\\par}`).join("\n")}`;
 ...
 \\begin{itemize}[leftmargin=0.15in, label={}]
    \\small{\\item{
${allBullets.map((bullet) => `      ${renderSkillBullet(bullet, style.boldSkillLabels)} ${skillBreak(style)}`).join("\n")}
    }}
 \\end{itemize}`;
  }
